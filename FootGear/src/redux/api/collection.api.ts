...
export const fetchSneakers = createAsyncThunk<ISneakers[], ParamsSnk>(
  ...
  async (params) => {
    ...
    const { data } = await axios.get<ISneakers[]>(`https://653c2e20d5d6790f5ec7dea2.mockapi.io/v2/collection?page=${currentPage}&sortBy=${sort}&order=${order}`);
...
export const fetchFullSnk = createAsyncThunk<ISneakers, ParamsFull>(
  ...
  async (params) => {
    const { id } = params
    const { data } = await axios.get<ISneakers>(`https://653c2e20d5d6790f5ec7dea2.mockapi.io/v2/collection/${id}`);
    return data as ISneakers
  }
)